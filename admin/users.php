...
//获取昨天和今天注册人数
		...
		$sql="select count(*) as count from ".jieqi_dbprefix("system_users")." where regdate between $yesterday and $today";
		$query = JieqiQueryHandler::getInstance("JieqiQueryHandler");
		$yecount=$query->getRow($query->execute($sql));
		$sql="select count(*) as count from ".jieqi_dbprefix("system_users")." where regdate > $today";		
		$tcount=$query->getRow($query->execute($sql));
...
$jieqiTpl->assign_by_ref("tcount", $tcount['count']);
$jieqiTpl->assign_by_ref("yecount", $yecount['count']);
