...
//		echo '<br><br>';
	...
	protected function curl($url, $postFields = null) {
		...
		echo '<br><br>$reponse11 = ';
		
		var_dump(curl_errno($reponse));
...
//		$sysParams["format"] = $this->format;
		...
		$sysParams["method"] = $request->getApiMethodName();
//		$sysParams["timestamp"] = date("Y-m-d H:i:s");
		$sysParams["timestamp"] = '2017-01-17 23:42:07';
...
//		echo '<br><br>';
		...
		$signParams = array_merge($sysParams,$apiParams);
		
//		echo '<br><br>$signParams = ';
//
//		var_dump($signParams);
//
//		echo '<br><br>';
			
		$str = $this->getSignContent($signParams);
...
//		$sysParams["sign"] = $this->generateSign(array_merge($apiParams, $sysParams), $this->signType);
		...
		$signParams["sign"] = $this->generateSign($str, $this->signType);

//		echo '<br><br>$sysParams1 = ';
//
//		var_dump($sysParams);
//
//		echo '<br><br>';
...
//			$tmp .= "$sysParamKey=" . urlencode($this->characet($sysParamValue, $this->postCharset)) . "&";
//		}
		
		$testStr = $this->getSignContent($signParams);
		...
		foreach ($signParams as $sysParamKey => $sysParamValue)
		{
			$tmp .= "$sysParamKey=" . urlencode($this->characet($sysParamValue, $this->postCharset)) . "&";
		}
...
//		}
//
//		echo '<br><br>$respWellFormed = ';
//
//		var_dump($respWellFormed);
//
//		echo '<br><br>';
//
...
//		echo '<br><br>';
		...
		if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
			echo '<br><br>checkResponseSign---1';
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				echo '<br><br>checkResponseSign---5';
				
				echo '<br><br>$checkResult = ';
				...
				if (!$checkResult)
				{
					...
					echo '<br><br>checkResponseSign---6';
					
					if (strpos($signData->signSourceData, "\\/") > 0)
					{
						
						echo '<br><br>checkResponseSign---7';
						
						$signData->signSourceData = str_replace("\\/", "/", $signData->signSourceData);

						$checkResult = $this->verify($signData->signSourceData, $signData->sign, $this->alipayPublicKey, $this->signType);
						
						echo '<br><br>checkResponseSign---8';
	...
	public function burnExcute($request, $authToken = null, $appInfoAuthtoken = null)
	{
		$this->setupCharsets($request);
