...
class JieqiPage extends JieqiObject
{
	...
	public $pagerows;//每页显示条数
	public $pagelinks;//分页连接
	public $current;//当前页
	public $totalpages;//总页数
	public $linkurl;//链接地址
	public $pagevar;//地址参数
	public $useajax;//使用ajax
...
//			$linkbar .= $linkchar;
	...
	public function jump_form()
	{
		...
		if ($this->useajax == 1) {
			$form = "<input name=\"page\" type=\"text\" size=\"4\" maxlength=\"6\" onkeydown=\"if(event.keyCode==13){Ajax.Update(".$urlcode.",".$this->ajax_parm."); return false;}\" /></kbd>";
		}
		else {
			$form ="<input name=\"page\" type=\"text\" size=\"3\" maxlength=\"4\" onkeydown=\"if(event.keyCode==13){window.location=";
			$form .=$urlcode;
			$form .="; return false;}\" /></kbd>";
		}
