...
function jieqi_article_addchapter(&$postvars, &$attachvars, $article)
{
	...
	//增加章节时候如果书籍已经设置过千字/币,按千字多少币算
	$query = JieqiQueryHandler::getInstance("JieqiQueryHandler");
	$sql="SELECT peregold FROM ". jieqi_dbprefix("article_article")." WHERE articleid =". $article->getVar("articleid");	
	$query->execute($sql);
	$res=$query->getRow();
   //如果当前小说设置过千字价按千字价算	
	if($res["peregold"]>0){		
		$postvars["saleprice"]=round($chaptersize/(2*1000)*$res["peregold"]);		
	}
