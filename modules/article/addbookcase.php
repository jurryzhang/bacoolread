...
if (!empty($_REQUEST['cid'])) {
	...
	if (!$bookcase_handler->insert($bookcase)) {
        ...
        $articlea_handler = &JieqiArticleHandler::getInstance('JieqiArticleHandler');
        $articlea = $articlea_handler->get($chapter->getVar('articleid', 'n'));
		include_once $jieqiModules['article']['path'] . '/include/funaction.php';
        $actions = array('actname' => 'bookcase', 'actnum' => $addnum);
        jieqi_article_actiondo($actions, $articlea);
	...
	if (!$bookcase_handler->insert($bookcase)) {
		...
		include_once $jieqiModules['article']['path'] . '/class/article.php';
        $articleb_handler = &JieqiArticleHandler::getInstance('JieqiArticleHandler');
        $articleb = $articleb_handler->get($chapter->getVar('articleid', 'n'));
		include_once $jieqiModules['article']['path'] . '/include/funaction.php';
        $actions = array('actname' => 'bookcase', 'actnum' => $addnum);
        jieqi_article_actiondo($actions, $articleb);
		jieqi_msgwin(LANG_DO_SUCCESS, $jieqiLang['article']['add_chaptermark_success']);
	}
}
