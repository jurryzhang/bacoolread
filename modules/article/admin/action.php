...
$power_handler->getSavedVars("article");
...
/*case "new":
	...
	if (empty($_POST["acttitle"])) {
		$errtext .= $jieqiLang["article"]["need_action_acttitle"] . "<br />";
	}
	...
	if (!is_numeric($_POST["minscore"]) || !is_numeric($_POST["islog"]) || !is_numeric($_POST["isreview"]) || !is_numeric($_POST["isvip"]) || !is_numeric($_POST["ispay"]) || !is_numeric($_POST["ismessage"]) || !is_numeric($_POST["paymin"]) || !is_numeric($_POST["paymax"]) || !is_numeric($_POST["earnscore"]) || !is_numeric($_POST["earncredit"]) || !is_numeric($_POST["earnvipvote"])) {
		$errtext .= $jieqiLang["article"]["need_action_num"] . "<br />";
	}
	...
	if ($_POST["paymin"] < $_POST["paymax"]) {
		$errtext .= $jieqiLang["article"]["max_than_min"] . "<br />";
	}
	...
	if (empty($errtext)) {
		...
		if (!$action_handler->insert($action)) {
			jieqi_printfail($jieqiLang["article"]["add_action_failure"]);
		}
	}
	else {
	...
	if (!empty($_REQUEST["id"]) && !empty($_POST["acttitle"])) {
		...
		if (is_object($action)) {
			...
			if (empty($errtext)) {
				...
				$action->setVar("isvip", $_POST["isvip"]);
				$action->setVar("ispay", $_POST["ispay"]);
				$action->setVar("ismessage", $_POST["ismessage"]);
				$action->setVar("paytitle", $_POST["paytitle"]);
				$action->setVar("paymin", $_POST["paymin"]);
				$action->setVar("paymax", $_POST["paymax"]);
				$action->setVar("earnscore", $_POST["earnscore"]);
				...
				$action->setVar("earnvipvote", $_POST["earnvipvote"]);
				if (!$action_handler->insert($action)) {
					jieqi_printfail($jieqiLang["article"]["edit_action_failure"]);
				}
			}
			else {
				jieqi_printfail($errtext);
...
case "edit":
	if (!empty($_REQUEST["id"])) {
		...
		if (is_object($action)) {
			...
			$action_form = new JieqiThemeForm($jieqiLang["article"]["edit_action"], "actionedit", JIEQI_URL . "/modules/article/admin/action.php");
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_acttitle"], "acttitle", 30, 250, $action->getVar("acttitle", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_minscore"], "minscore", 30, 50, $action->getVar("minscore", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_islog"], "islog", 30, 50, $action->getVar("islog", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_isreview"], "isreview", 30, 50, $action->getVar("isreview", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_isvip"], "isvip", 30, 50, $action->getVar("isvip", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_ispay"], "ispay", 30, 50, $action->getVar("ispay", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_ismessage"], "ismessage", 30, 50, $action->getVar("ismessage", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_paytitle"], "paytitle", 30, 50, $action->getVar("paytitle", "e")));
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_paymin"], "paymin", 30, 50, $action->getVar("paymin", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_paymax"], "paymax", 30, 50, $action->getVar("paymax", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_earnscore"], "earnscore", 30, 50, $action->getVar("earnscore", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_earncredit"], "earncredit", 30, 50, $action->getVar("earncredit", "e")), true);
			$action_form->addElement(new JieqiFormText($jieqiLang["article"]["table_action_earnvipvote"], "earnvipvote", 30, 50, $action->getVar("earnvipvote", "e")), true);
...
while ($rows = $action_handler->getObject()) {
	...
	$actionary[JIEQI_MODULE_NAME][$rows->getVar("acttype")] = array("acttitle" => $rows->getVar("acttitle"), "minscore" => $rows->getVar("minscore"), "islog" => $rows->getVar("islog"), "isreview" => $rows->getVar("isreview"), "isvip" => $rows->getVar("isvip"), "ismessage" => $rows->getVar("ismessage"), "ispay" => $rows->getVar("ispay"), "paytitle" => $rows->getVar("paytitle"), "paybase" => 1, "paymin" => $rows->getVar("paymin"), "paymax" => $rows->getVar("paymax"), "earnscore" => $rows->getVar("earnscore"), "earncredit" => $rows->getVar("earncredit"), "earnvipvote" => $rows->getVar("earnvipvote"));
	$i++;
	if ((!empty($_REQUEST["id"]) || !empty($_POST["acttitle"])) && (0 < count($actionary))) {
	jieqi_setconfigs("action", "jieqiAction", $actionary, "article");
}
}
...
$jieqiTset["jieqi_contents_template"] = $jieqiModules["article"]["path"] . "/templates/admin/action.html";
include_once (JIEQI_ROOT_PATH . "/admin/footer.php");
