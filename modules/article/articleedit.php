...
case "update":
	...
	if(isset($_POST['freetime'])){
		$article->setVar("freetime", strtotime($_POST["freetime"]));
	}
		//edit by muyi 添加设置千字X币功能
		if(!empty($_POST['peregold'])&&is_numeric($_POST['peregold'])){
			$peregold=intval($_POST['peregold']);
			if($peregold>0){
			//添加到小说表中
			$article->setVar("peregold", $peregold);
			//查找该小说对应的收费章节,设置千字X币
			$articleid=$article->getVar('articleid');
			$query = JieqiQueryHandler::getInstance("JieqiQueryHandler");
			$sql="SELECT ochapterid,size FROM ". jieqi_dbprefix("obook_ochapter")." where articleid = ".$articleid;//获取收费章节	
				if($res=$query ->execute($sql)){
			//更改收费章节价格
					while ($row = mysql_fetch_assoc($res)) {
					  $saleprice=round($row['size']/(2*1000)*$peregold);
					  $sql="UPDATE ". jieqi_dbprefix("obook_ochapter")." SET saleprice = ".$saleprice." WHERE ochapterid = ".$row['ochapterid'];
					  $query ->execute($sql);			 
					}
					$sql="SELECT chapterid,size FROM ".jieqi_dbprefix("article_chapter")." WHERE articleid = ".$articleid;
					if($res=$query ->execute($sql)){
				//更改所有章节的价格
						while ($row = mysql_fetch_assoc($res)) {
						  $saleprice=round($row['size']/(2*1000)*$peregold);
						  $sql="UPDATE ". jieqi_dbprefix("article_chapter")." SET saleprice = ".$saleprice." WHERE chapterid = ".$row['chapterid'];
						  $query ->execute($sql);			 
						}
					}

					$sql2 = "SELECT chapterid,size FROM ".jieqi_dbprefix("distribution_obook_ochapter")." WHERE articleid = ".$articleid;
					if($res2=$query ->execute($sql2)){
					//更改所有章节的价格
						while ($row = mysql_fetch_assoc($res2)) {
						  $saleprice=round($row['size']/(2*1000)*$peregold);
						  $sql3="UPDATE ". jieqi_dbprefix("distribution_obook_ochapter")." SET saleprice = ".$saleprice." WHERE chapterid = ".$row['chapterid'];
						  $query ->execute($sql3);			 
						}
					}
