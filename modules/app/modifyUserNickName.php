...
switch ($userInfo)
{
	...
	default :
	{
		...
		$name   = iconv("UTF-8","GBK//IGNORE",$userNickname);
		
		$sql    = 'UPDATE ' . jieqi_dbprefix('system_users'). "  SET  name = '" . $name . "' WHERE uid = " . $userID ."";
...
if(!$errorMessage)
{
	echo json_encode_ex(array('status' => $status,'message' => '修改用户昵称成功！','result' => $userResult));
