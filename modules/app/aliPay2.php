...
if(isset($_REQUEST['gold_type']) && is_numeric($_REQUEST['gold_type']) && $_REQUEST['gold_type'] > 0)
{
	...
	$_REQUEST['gold_type'] = intval($_REQUEST['gold_type']);
	
	if(!empty($aliPay['paylimit']))
	{
		if(!empty($aliPay['paylimit'][$_REQUEST['gold_type']]))
		{
			$money = intval($aliPay['paylimit'][$_REQUEST['gold_type']]);
		}
		else
		{
...
//	$paylog->setVar('money', '0.01');
	$paylog->setVar('gold_typetype', $aliPay['paysilver']);
	$paylog->setVar('gold_type', $_REQUEST['gold_type']);
