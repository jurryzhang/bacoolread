...
switch ($userInfo)
{
	...
	default :
	{
		//关闭支付，egold字段改为gold_type_type
		
		if(isset($_REQUEST['gold_type']) && is_numeric($_REQUEST['gold_type']) && $_REQUEST['gold_type'] > 0)
		{
			$gold_type = intval($_REQUEST['gold_type']);
			
			//计算实际需要支付的金额
			if(!empty($weixinPay['paylimit']))
			{
				if(!empty($weixinPay['paylimit'][$gold_type]))
				{
					$money = intval($weixinPay['paylimit'][$gold_type]);
				}
				else
				{
			...
			{
				$money = intval($_REQUEST['gold_type']);
			}
...
//			$paylog->setVar('money', 1);//测试
			...
			$paylog->setVar('gold_typetype', $weixinPay['paysilver']);
			$paylog->setVar('gold_type', $gold_type);
