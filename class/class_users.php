...
$system_users_fields['juan'] = array('name' => 'juan', 'type' => JIEQI_TYPE_INT, 'value' => 0, 'caption' => '赠券', 'required' => false, 'maxlength' => 1);
...
//赠眷	
	public function juan(&$users, $juan)
	{
		$juan = intval($juan);
		...
		$userejuan = intval($tmpuser->getVar('juan', 'n'));
		$useresilver = intval($tmpuser->getVar('esilver', 'n'));
		$expenses = intval($tmpuser->getVar('expenses', 'n'));
		$useremoney = $useregold + $useresilver;
		...
		if ($juan <= $userejuan) {
			$tmpuser->setVar('juan', $userejuan - $juan);
		}

		$tmpuser->setVar('expenses', $expenses + $juan);
		if (!empty($_SESSION['jieqiUserId']) && ($tmpuser->getVar('uid', 'n') == $_SESSION['jieqiUserId'])) {
			$tmpuser->saveToSession();
		}
