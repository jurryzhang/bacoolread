...
function jieqi_jumppage($url, $title = '', $content = '', $direct = false)
{
	...
	if(!empty($_REQUEST['ajax_layer'])) {
		header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
		header('Cache-Control:no-cache');
		echo $_REQUEST["CALLBACK"] . "(" . json_encode(array("status" => "OK", "msg" => iconv('gb2312','utf-8',$content), "jumpurl" => $url)) . ")";
	}else
	if (empty($_REQUEST['ajax_request'])) {
		if (($direct == true) || empty($content)) {
			...
			if ((JIEQI_VERSION_TYPE != '') && (JIEQI_VERSION_TYPE != 'Free')) {
				...
				$jieqiTpl = &JieqiTpl::getInstance();
				if($_SERVER["HTTP_REFERER"] == JIEQI_WAP_URL.'/login.php' || $_SERVER["HTTP_REFERER"] == JIEQI_WAP_URL.'/register.php'){
				$source = JIEQI_WAP_URL;
				}else{
				$source = end(explode('jumpurl=', rawurldecode($_SERVER["HTTP_REFERER"])));
				}
				$jieqiTpl->assign(array('jieqi_charset' => JIEQI_CHAR_SET, 'jieqi_themeurl' => JIEQI_URL . '/themes/' . JIEQI_THEME_NAME . '/', 'jieqi_themecss' => JIEQI_URL . '/themes/' . JIEQI_THEME_NAME . '/style.css', 'pagetitle' => $title, 'title' => $title, 'content' => $content, 'url' => $url, 'jieqi_wap_url' => JIEQI_WAP_URL , 'source' => $source ));
				$jieqiTpl->setCaching(0);
...
function jieqi_msgwin($title, $content)
{
    ...
    if(!empty($_REQUEST['ajax_layer'])) {
		header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
		header('Cache-Control:no-cache');
		echo $_REQUEST["CALLBACK"] . "(" . json_encode(array("status" => "OK", "msg" => iconv('gb2312','utf-8',$content), "jumpurl" => "")) . ")";
	}else
	if (empty($_REQUEST['ajax_request'])) {
		include_once JIEQI_ROOT_PATH . '/lib/template/template.php';
		$title = jieqi_htmlstr($title);
...
function jieqi_printfail($errorinfo)
{
	...
	if(!empty($_REQUEST['ajax_layer'])) {
		header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
		header('Cache-Control:no-cache');
		echo $_REQUEST["CALLBACK"] . "(" . json_encode(array("status" => "NO", "msg" => iconv('gb2312','utf-8',$errorinfo), "jumpurl" => "")) . ")";
	}else 
	if (empty($_REQUEST['ajax_request'])) {
		include_once JIEQI_ROOT_PATH . '/lib/template/template.php';
		$jieqiTpl = &JieqiTpl::getInstance();
	...
	if ($ustatus == JIEQI_GROUP_ADMIN) {
		...
		if (is_array($powerset['groups']) && (in_array($ugroup, $powerset['groups'], false) || in_array('0', $powerset['groups'], false))) {
			...
			if(!empty($_REQUEST['ajax_layer'])) {
		        header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
		        header('Cache-Control:no-cache');
		        return (json_encode(array("status" => "OK", "msg" => iconv('gb2312','utf-8',LANG_NEED_LOGIN), "jumpurl" => urlencode($jumpurl))));
	        }else
			if (empty($_REQUEST['ajax_request'])) {
				if ($isadmin) {
	...
	if ($isreturn) {
		...
		if (empty($_REQUEST['ajax_request'])) {
			...
			if ($isadmin) {
				...
				header('Location: ' . jieqi_headstr(JIEQI_WAP_URL . '/login.php?jumpurl=' . urlencode($jumpurl)));
			}
		}
		else {
			header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
...
if (empty($jieqi_license_modules) || ((JIEQI_LOCAL_HOST == '') && (JIEQI_PHP_CLI == 1) && (ALLOW_PHP_CLI == 1)) || preg_match('/^' . preg_quote(str_replace(array('\\\\', '\\'), '/', JIEQI_ROOT_PATH), '/') . '\\/(admin|install|logout\\.php)/is', JIEQI_SCRIPT_FILENAME) || preg_match('/^(http:\\/\\/|https:\\/\\/)?[^\\/\\?]*(localhost|127.0.0.1)/i', JIEQI_LOCAL_HOST, $matchs)) {
	...
	if (!empty($jieqi_license_ary[1]) && preg_match('/^(http:\\/\\/|https:\\/\\/)?[^\\/\\?]*(' . $jieqi_license_ary[1] . ')/i', JIEQI_LOCAL_HOST, $matchs)) {
		...
		$tmpvar = md5(base64_encode(sha1($jieqi_license_ary[1] . $jieqi_license_ary[2] . chr(32) . 'yscp' . JIEQI_TARGET_NEW . chr(88) . '13' . JIEQI_TARGET_TOP . 'dyxz' . chr(66) . '52DDjc', true)), true);
...
foreach ($jieqiModules as $k => $v) {
	...
	if ($v['theme'] == '') {
		$jieqiModules[$k]['theme'] = JIEQI_THEME_WAP;
	}
...
if (!defined('JIEQI_THEME_NAME')) {
	define('JIEQI_THEME_NAME', JIEQI_THEME_WAP);
}
...
$mobile = JIEQI_MOBILE_URL;

if (!empty($mobile)) {
	define('JIEQI_WAP_URL', JIEQI_MOBILE_URL);
}
else {
	define('JIEQI_WAP_URL', JIEQI_URL.'/wap');
}
...
if ((JIEQI_CHAR_SET != JIEQI_SYSTEM_CHARSET) || ((!empty($_REQUEST['ajax_request']) || !empty($_REQUEST['ajax_layer'])) && ($charsetary[JIEQI_CHAR_SET] != 'utf8'))) {
	include_once JIEQI_ROOT_PATH . '/include/changecode.php';
}

if ((!empty($_REQUEST['ajax_request']) || !empty($_REQUEST['ajax_layer'])) && ($charsetary[JIEQI_CHAR_SET] != 'utf8')) {
...
if ((JIEQI_SYSTEM_CHARSET != JIEQI_CHAR_SET) || ((!empty($_REQUEST['ajax_request']) || !empty($_REQUEST['ajax_layer'])) && ($charsetary[JIEQI_CHAR_SET] != 'utf8'))) {
